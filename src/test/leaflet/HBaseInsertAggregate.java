...
public class HBaseInsertAggregate {
        ...
        public void insertDetail() throws Exception {
                ...
                Map<String, Map<String, Map<String, Trip>>> group = new HashMap<String, Map<String, Map<String, Trip>>>();
                {
                ...
                for (String medallion: group.keySet()) {
                        ...
                        for (String date: group.get(medallion).keySet()) {
                                ...
                                for (String datetime: group.get(medallion).get(date).keySet()) {
                                        ...
                                        tripJson.add("trip:passenger_count", new JsonPrimitive(trip.get("trip:passenger_count")));
                                        tripJson.add("trip:trip_time_in_secs", new JsonPrimitive(trip.get("trip:trip_time_in_secs")));
                                        tripJson.add("trip:trip_distance", new JsonPrimitive(trip.get("trip:trip_distance")));
                                        tripJson.add("trip:pickup_location", new JsonArray());
                                        tripJson.get("trip:pickup_location").getAsJsonArray().add(new JsonPrimitive(trip.get("trip:pickup_longitude")));
                                        tripJson.get("trip:pickup_location").getAsJsonArray().add(new JsonPrimitive(trip.get("trip:pickup_latitude")));
                                        tripJson.add("trip:dropoff_location", new JsonArray());
                                        tripJson.get("trip:dropoff_location").getAsJsonArray().add(new JsonPrimitive(trip.get("trip:dropoff_longitude")));
                                        tripJson.get("trip:dropoff_location").getAsJsonArray().add(new JsonPrimitive(trip.get("trip:dropoff_latitude")));
                                        days.get(date).getAsJsonArray().add(tripJson);
                                }
                        }
